...
bool KinectV1Handler::initKinect() {
    ...
    kinectSensor->NuiSkeletonTrackingEnable(NULL, 0); //NUI_SKELETON_TRACKING_FLAG_ENABLE_IN_NEAR_RANGE
...
void KinectV1Handler::updateSkeletalData() {
    if (kinectSensor->NuiSkeletonGetNextFrame(0, &skeletonFrame) >= 0) {
        ...
        for (int i = 0; i < NUI_SKELETON_COUNT; ++i) {
            ...
            {
                KinectHandlerBase::isSkeletonTracked = true;
                for (int j = 0; j < NUI_SKELETON_POSITION_COUNT; ++j) {
                    ...
                    KinectHandlerBase::trackingStates[j] = skeletonFrame.SkeletonData[i].eSkeletonPositionTrackingState[j];
                }
                ...
                for(int k = 0; k < NUI_SKELETON_POSITION_COUNT; ++k){
                    KinectHandlerBase::boneOrientations[k].w = boneOrientations[k].absoluteRotation.rotationQuaternion.w;
                    KinectHandlerBase::boneOrientations[k].x = boneOrientations[k].absoluteRotation.rotationQuaternion.x;
                    KinectHandlerBase::boneOrientations[k].y = boneOrientations[k].absoluteRotation.rotationQuaternion.y;
                    KinectHandlerBase::boneOrientations[k].z = boneOrientations[k].absoluteRotation.rotationQuaternion.z;
                }
                break;
            }
            else
                KinectHandlerBase::isSkeletonTracked = false;
        }
