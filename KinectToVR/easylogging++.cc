...
namespace el
{
	...
	namespace base
	{
		...
		namespace consts
		{
			...
			static const type::char_t* kWarningLevelLogValue = ELPP_LITERAL("WARNING");
			...
			static const type::char_t* kVerboseLevelLogValue =
...
static const char* kLoggingFlagsParam                      =      "--logging-flags";
  ...
  _asm int 3
	...
	void Configuration::log(base::type::ostream_t& os) const
	{
  ...
  unsafeSetIfNotExist(Level::Global, ConfigurationType::Enabled, std::string("false"));
	...
	namespace base
	{
		...
		namespace utils
		{
			...
			type::fstream_t* File::newFileStream(const std::string& filename)
			{
...
#  endif // ELPP_OS_WINDOWS
  fs->imbue(elppUnicodeLocale);
			...
			std::size_t File::getSizeOfFile(type::fstream_t* fs)
			{
				if (fs == nullptr)
				{
					return 0;
  ...
  currPath = STRTOK(currPath, base::consts::kFilePathSeperator, 0);
    ...
    currPath = STRTOK(nullptr, base::consts::kFilePathSeperator, 0);
			...
			{
				return (str.length() >= end.length()) && (str.compare(str.length() - end.length(), end.length(), end) ==
					0);
			}
			...
			void Str::replaceFirstWithEscape(type::string_t& str, const type::string_t& replaceWhat,
...
#  endif  // ELPP_COMPILER_MSVC || defined(_MSC_EXTENSIONS)
  ...
  ::gettimeofday(tv, nullptr);
			...
			std::string DateTime::getDateTime(const char* format, const SubsecondPrecision* ssPrec)
			{
			...
			std::string DateTime::timevalToString(struct timeval tval, const char* format,
			                                      const SubsecondPrecision* ssPrec)
			{
				struct tm timeInfo;
			...
			type::string_t DateTime::formatTime(unsigned long long time, TimestampUnit timestampUnit)
			{
				...
				{
					if (time <= consts::kTimeFormats[i].value)
					{
						break;
					}
					if (consts::kTimeFormats[i].value == 1000.0f && time / 1000.0f < 1.9f)
					{
						break;
					}
					time /= static_cast<decltype(time)>(consts::kTimeFormats[i].value);
...
#  endif  // ELPP_COMPILER_MSVC
			...
			char* DateTime::parseFormat(char* buf, std::size_t bufSz, const char* format, const struct tm* tInfo,
			                            std::size_t msec, const SubsecondPrecision* ssPrec)
			{
				...
				for (; *format; ++format)
				{
					if (*format == consts::kFormatSpecifierChar)
					{
						...
						case 'g':
							buf = Str::convertAndAddToBuff(msec, ssPrec->m_width, buf, bufLim);
							continue;
						case 'F': // AM/PM
			...
			type::ostream_t& operator<<(type::ostream_t& os, const CommandLineArgs& c)
			{
...
#  endif  // !ELPP_USE_STD_THREADING
		...
		void SubsecondPrecision::init(int width)
		{
			if (width < 1 || width > 6)
			{
				width = consts::kDefaultSubsecondPrecision;
			}
			m_width = width;
			switch (m_width)
			{
		...
		LogFormat::LogFormat(void) :
			...
			m_flags(0x0),
			m_currentUser(utils::OS::currentUser()),
			m_currentHost(utils::OS::currentHost())
		{
		}

		LogFormat::LogFormat(Level level, const type::string_t& format)
			: m_level(level), m_userFormat(format), m_currentUser(utils::OS::currentUser()),
			  m_currentHost(utils::OS::currentHost())
		{
		...
		void LogFormat::parseFromFormat(const type::string_t& userFormat)
		{
		...
		void LogFormat::updateDateFormat(std::size_t index, type::string_t& currFormat)
		{
		...
		const LogFormat& TypedConfigurations::logFormat(Level level)
		{
			return getConfigByRef<LogFormat>(level, &m_logFormatMap, "logFormat");
		}

		const SubsecondPrecision& TypedConfigurations::subsecondPrecision(Level level)
		{
			return getConfigByRef<SubsecondPrecision>(level, &m_subsecondPrecisionMap, "subsecondPrecision");
		}

		const MillisecondsWidth& TypedConfigurations::millisecondsWidth(Level level)
		{
			return getConfigByRef<MillisecondsWidth>(level, &m_subsecondPrecisionMap, "millisecondsWidth");
		}
		...
		type::fstream_t* TypedConfigurations::fileStream(Level level)
		{
			...
			for (Configurations::const_iterator it = configurations->begin(); it != configurations->end(); ++it)
			{
				...
				else if (conf->configurationType() == ConfigurationType::Format)
				{
					setValue(conf->level(), base::LogFormat(conf->level(),
					                                        type::string_t(conf->value().begin(), conf->value().end())),
					         &m_logFormatMap);
				}
		...
		std::string TypedConfigurations::resolveFilename(const std::string& filename)
		{
			...
			if ((dateIndex = resultingFilename.find(dateTimeFormatSpecifierStr.c_str())) != std::string::npos)
			{
				...
				if (dateIndex != std::string::npos)
				{
					...
					std::string now = utils::DateTime::getDateTime(fmt.c_str(), &ssPrec);
			...
			auto create = [&](Level level)
			{
				...
				if (filestreamIter == m_logStreamsReference->end())
				{
					...
					m_logStreamsReference->insert(
						std::make_pair(resolvedFilename, FileStreamPtr(m_fileStreamMap.at(level))));
				}
				else
				{
		...
		bool RegisteredHitCounters::validateEveryN(const char* filename, type::LineNumber lineNumber, std::size_t n)
		{
			threading::ScopedLock scopedLock(lock());
		...
		bool RegisteredHitCounters::validateAfterN(const char* filename, type::LineNumber lineNumber, std::size_t n)
		{
			threading::ScopedLock scopedLock(lock());
			HitCounter* counter = get(filename, lineNumber);
			if (counter == nullptr)
		...
		/// @return True if validation resulted in triggering hit. Meaning logs should be written everytime true is returned
		bool RegisteredHitCounters::validateNTimes(const char* filename, type::LineNumber lineNumber, std::size_t n)
		{
			threading::ScopedLock scopedLock(lock());
			HitCounter* counter = get(filename, lineNumber);
			if (counter == nullptr)
		...
		Logger* RegisteredLoggers::get(const std::string& id, bool forceCreation)
		{
			...
			if (logger_ == nullptr && forceCreation)
			{
				...
				for (const std::pair<std::string, type::LoggerRegistrationCallbackPtr>& h
		...
		VRegistry::VRegistry(type::VerboseLevel level, type::EnumType* pFlags) : m_level(level), m_pFlags(pFlags)
		{
		}

		/// @brief Sets verbose level. Accepted range is 0-9
		void VRegistry::setLevel(type::VerboseLevel level)
		{
			threading::ScopedLock scopedLock(lock());
			if (level > 9)
		...
		void VRegistry::setModules(const char* modules)
		{
			...
			for (; *modules; ++modules)
			{
				...
				case ',':
					...
					{
						insert(ss, static_cast<type::VerboseLevel>(level));
						ss.str(std::string(""));
						level = -1;
					}
					break;
		...
		bool VRegistry::allowed(type::VerboseLevel vlevel, const char* file)
		{
			threading::ScopedLock scopedLock(lock());
			...
			char baseFilename[consts::kSourceFilenameMaxLength] = "";
			utils::File::buildBaseFilename(file, baseFilename);
			std::unordered_map<std::string, type::VerboseLevel>::iterator it = m_modules.begin();
			for (; it != m_modules.end(); ++it)
			{
				if (utils::Str::wildCardMatch(baseFilename, it->first.c_str()))
				{
		...
		void VRegistry::setFromArgs(const utils::CommandLineArgs* commandLineArgs)
		{
			...
			{
				setLevel(static_cast<type::VerboseLevel>(atoi(commandLineArgs->getParamValue("--v"))));
			}
			else if (commandLineArgs->hasParamWithValue("--V"))
			{
				setLevel(static_cast<type::VerboseLevel>(atoi(commandLineArgs->getParamValue("--V"))));
...
Storage::Storage(const LogBuilderPtr& defaultLogBuilder, base::IWorker* asyncDispatchWorker) :
  ...
  base::utils::safeDelete(m_asyncLogQueue);
		...
		{
			threading::ScopedLock scopedLock(customFormatSpecifiersLock());
  ...
  base::threading::ScopedLock scopedLock(fileHandle(data));
		...
		void DefaultLogDispatchCallback::dispatch(type::string_t&& logLine)
		{
...
void AsyncDispatchWorker::run(void) {
  while (continueRunning()) {
    ...
    base::threading::msleep(10); // 10ms
		...
		type::string_t DefaultLogBuilder::build(const LogMessage* logMessage, bool appendNewLine) const
		{
			...
			if (logFormat->hasFlag(FormatFlags::ThreadId))
			{
				...
				utils::Str::replaceFirstWithEscape(logLine, consts::kThreadIdFormatSpecifier,
				                                   ELPP->getThreadName(threading::getCurrentThreadId()));
			}
			...
			{
				// File
				utils::Str::clearBuff(buff, consts::kSourceFilenameMaxLength);
			...
			if (logFormat->hasFlag(FormatFlags::Line))
			{
				...
				buf = utils::Str::convertAndAddToBuff(logMessage->line(), consts::kSourceLineMaxLength, buf, bufLim,
				                                      false);
			...
			threading::ScopedLock lock_(ELPP->customFormatSpecifiersLock());
			ELPP_UNUSED(lock_);
			     ...
			     it != ELPP->customFormatSpecifiers()->end(); ++it)
			{
				...
				utils::Str::replaceFirstWithEscape(logLine, wcsFormatSpecifier, it->resolver()(logMessage));
			}
			...
			if (m_proceed)
			{
				if (m_msg == nullptr)
				{
					LogMessage msg(m_level, m_file, m_line, m_func, m_verboseLevel,
					               m_logger);
					LogDispatcher(m_proceed, &msg, m_dispatchAction).dispatch();
				}
				else
				{
					LogDispatcher(m_proceed, m_msg, m_dispatchAction).dispatch();
				}
			}
			if (m_logger != nullptr)
			{
...
void Helpers::logCrashReason(int sig, bool stackTraceIfAvailable, Level level, const char* logger) {
  el::base::debug::logCrashReason(sig, stackTraceIfAvailable, level, logger);
	...
	void Loggers::setDefaultLogBuilder(LogBuilderPtr& logBuilderPtr)
	{
